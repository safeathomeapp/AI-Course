(function () {
  const SPRITE_ID = 'shared-icon-sprite-defs';
  const SPRITE_CLASS = 'shared-icon-sprite-defs';
  const FALLBACK_SPRITE_MARKUP = `<svg xmlns="http://www.w3.org/2000/svg">
  ...
  <symbol id="icon-circle-question-mark" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  function createSprite(markup) {
    ...
    if (!sprite) {
      throw new Error('Shared icon sprite did not contain an SVG root.');
    }
  ...
  async function injectSprite() {
    ...
    const currentScript = document.currentScript || document.querySelector('script[data-shared-icon-sprite]');

    if (!currentScript) {
      return;
    }

    const spriteUrl = currentScript.dataset.spriteUrl || new URL('../icons/sprite.svg', currentScript.src).toString();
    let sprite;
    ...
    if (window.location.protocol !== 'file:') {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Unable to load shared icon sprite: ${response.status}`);
        }

        sprite = createSprite(await response.text());
      } catch {
        sprite = createSprite(FALLBACK_SPRITE_MARKUP);
      }
    } else {
      sprite = createSprite(FALLBACK_SPRITE_MARKUP);
    }
